...
module.exports = {
  login: async (req, res) => {
    try {
      ...
      if (!resultAdmin) {
        return res
          .status(404)
          .send({ isError: true, message: "Invalid email or password" });
      }
      bcrypt.compare(password, resultAdmin.password, async (err, result) => {
        if (err) {
          res.status(404).send({
            isError: true,
            message: "Login failed when comparing password",
          });
        } else if (result) {
          const token = jwt.sign(
            {
              id_admin: resultAdmin.id,
          ...
          await admins.update(
            { token_admin: token },
            { where: { id: resultAdmin.id } }
          );
          let getAdmin = await admins.findOne({
            where: { id: resultAdmin.id },
          });
          delete getAdmin.dataValues.password;
          res
            .status(200)
            .send({ isError: false, message: "Login Success", data: getAdmin });
        } else {
          return res
            .status(404)
            .send({ isError: true, message: "Invalid email or password" });
  ...
  createNewAdmin: async (req, res) => {
    try {
      ...
      if (!charEmail.test(email))
        return res
          .status(404)
          .send({ isError: true, message: "Invalid email format" });
  ...
  getAdminByToken: async (req, res) => {
    try {
      ...
      res.send({
        code: 200,
        message: "Get admin by token success",
        admin: getAdmin,
      });
    } catch (error) {
