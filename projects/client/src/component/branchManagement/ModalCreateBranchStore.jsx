            ...
            <Transition.Child as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enterTo="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 translate-y-0 sm:scale-100" leaveTo="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" >
              <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pt-5 pb-4 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
                ...
                <div className="mt-5 sm:mt-6 flex">
                  <button disabled={isLoading} type="button" className={`mr-2 inline-flex w-full justify-center rounded-md border border-transparent ${ isLoading ? "bg-gray-300 cursor-not-allowed" : "bg-green-600 hover:bg-green-700" } px-4 py-2 text-base font-medium text-white shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 sm:text-sm`} onClick={() => { postCreateBranchAdmin() }} > Add New Store </button>
